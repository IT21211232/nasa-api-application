...
export default function Navbar({navbarOptions, metaData}) {
  ...
  return (
    ...
    className={`w-full h-14 ${!background ? 'bg-transparent' : 'backdrop-filter backdrop-blur-md bg-[rgba(0,0,0,0.6)]'} fixed top-0 left-0 z-10 flex justify-end transition-all duration-700`}>
      <div className={`min-[880px]:opacity-0 min-[880px]:pointer-events-none fixed flex flex-col items-center justify-center transition-all duration-500 z-10 left-0 mobile_options h-screen w-[80vw] bg-[rgb(0,0,0)] ${navbarOpen ? 'translate-x-0' : 'translate-x-[-100vw]'}`}>
        {
      ...
      <img  src={NasaLogo} alt="logo image" className={`absolute left-2/4 -translate-x-1/2 -translate-y-1/2 filter invert transition-all duration-[2000ms] ${!stayDown ? 'h-[28px] top-2/4' : 'h-[90px] max-[880px]:h-[70px] top-[calc(50vh-100px)]'}`}/>
      <div className="h-full w-fit px-3 flex items-center opacity-100 max-[880px]:opacity-0 max-[880px]:pointer-events-none duration-200">
        {
        navbarOptions.map((data)=> (
      ...
      <div onClick={()=> {setNavbarOpen(!navbarOpen)}} className='min-[880px]:opacity-0 mr-3 w-fit h-full flex flex-col items-center justify-center transition-all duration-500'>
